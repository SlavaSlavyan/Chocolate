...
class SalesAnalyticsApp:
    """Приложение для анализа данных о продажах с устойчивостью к 'грязным' данным."""

    def __init__(self, root):
        self.root = root
        self.root.title("Аналитика продаж (устойчивая)")
    ...
    def load_csv(self):
        ...
        try:
            # Читаем без парсинга дат, чтобы избежать ошибок на этапе чтения
            df = pd.read_csv(filepath, dtype=str)  # всё как строки для безопасной очистки
            ...
            # 1. Обработка даты: пробуем парсить, некорректные -> NaT
            df['Order_Date'] = pd.to_datetime(df['Order_Date'], errors='coerce')

            # 2. Очистка числовых колонок: убираем всё, кроме цифр, точки и минуса
            def clean_numeric(series):
                # Удаляем все символы, которые не являются цифрами, точкой или минусом
            ...
            self.df = df
            total_rows = len(df)
            # Считаем, сколько строк имеют проблемы хотя бы в одной из важных колонок
            problem_mask = df[['Order_Date', 'Amount', 'Product', 'Country', 'Channel']].isna().any(axis=1)
    ...
    # ----------------------------------------------------------------------
    # Методы аналитики (все используют dropna для нужных колонок)
    # ----------------------------------------------------------------------
    def plot_monthly_revenue(self):
        if not self._check_df():
            return
        # Убираем строки, где дата или сумма невалидны
        df_clean = self.df.dropna(subset=['Order_Date', 'Amount'])
        if df_clean.empty:
        ...
        monthly = df_clean.groupby('Month')['Amount'].sum().reset_index()
    ...
    def plot_sales_by_product(self):
        if not self._check_df():
            return
        df_clean = self.df.dropna(subset=['Product', 'Amount'])
        if df_clean.empty:
            messagebox.showwarning("Нет данных", "Нет строк с продуктом и суммой.")
            return
        data = df_clean.groupby('Product')['Amount'].sum().sort_values(ascending=False)
        if data.empty:
    ...
    def plot_sales_by_country(self):
        if not self._check_df():
            return
        df_clean = self.df.dropna(subset=['Country', 'Amount'])
        if df_clean.empty:
            messagebox.showwarning("Нет данных", "Нет строк с страной и суммой.")
            return
        data = df_clean.groupby('Country')['Amount'].sum().sort_values(ascending=False)
        if data.empty:
            messagebox.showwarning("Нет данных", "После группировки нет данных.")
            return
    ...
    def plot_sales_by_channel(self):
        if not self._check_df():
            return
        df_clean = self.df.dropna(subset=['Channel', 'Amount'])
        if df_clean.empty:
            messagebox.showwarning("Нет данных", "Нет строк с каналом и суммой.")
            return
        data = df_clean.groupby('Channel')['Amount'].sum().sort_values(ascending=False)
        if data.empty:
            messagebox.showwarning("Нет данных", "После группировки нет данных.")
            return
    ...
    def plot_top_salespersons(self):
        if not self._check_df():
            return
        df_clean = self.df.dropna(subset=['Salesperson', 'Amount'])
        if df_clean.empty:
            messagebox.showwarning("Нет данных", "Нет строк с продавцом и суммой.")
            return
        data = df_clean.groupby('Salesperson')['Amount'].sum().sort_values(ascending=False).head(10)
        if data.empty:
            messagebox.showwarning("Нет данных", "После группировки нет данных.")
            return
    ...
    def plot_discount_vs_amount(self):
        if not self._check_df():
            return
        df_clean = self.df.dropna(subset=['Discount_Pct', 'Amount'])
        if df_clean.empty:
            messagebox.showwarning("Нет данных", "Нет строк со скидкой и суммой.")
            return

        fig, ax = plt.subplots(figsize=(10, 5))
        ax.scatter(df_clean['Discount_Pct'], df_clean['Amount'], alpha=0.5, c='blue')
    ...
    def plot_marketing_vs_boxes(self):
        if not self._check_df():
            return
        df_clean = self.df.dropna(subset=['Marketing_Spend', 'Boxes_Shipped'])
        if df_clean.empty:
            messagebox.showwarning("Нет данных", "Нет строк с расходами и коробками.")
            return

        fig, ax = plt.subplots(figsize=(10, 5))
        ax.scatter(df_clean['Marketing_Spend'], df_clean['Boxes_Shipped'], alpha=0.5, c='green')
    ...
    def plot_discount_distribution(self):
        if not self._check_df():
            return
        df_clean = self.df.dropna(subset=['Discount_Pct'])
        if df_clean.empty:
            messagebox.showwarning("Нет данных", "Нет строк со скидкой.")
            return

        fig, ax = plt.subplots(figsize=(10, 5))
        ax.hist(df_clean['Discount_Pct'], bins=20, color='orange', edgecolor='black')
